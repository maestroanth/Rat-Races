...
SDL_Surface *load_image( std::string filename )
{
    ...
    if( loadedImage != NULL )
    {
        ...
        {
            //Color key surface
            SDL_SetColorKey( optimizedImage, SDL_SRCCOLORKEY, SDL_MapRGB( optimizedImage->format, 255, 0xFF, 0xFF ) );
        }
    }
...
void Foo::show()
{
    ...
    if( status == FOO_RIGHT )
    {
        apply_surface( offSet, SCREEN_HEIGHT - FOO_HEIGHT, foo, screen, &clipsRight[ 1 ] );
    }
    else if( status == FOO_LEFT )
    {
        apply_surface( offSet, SCREEN_HEIGHT - FOO_HEIGHT, foo, screen, &clipsLeft[ 2 ] );
    }
}
